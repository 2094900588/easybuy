<template>
    <div>
        <div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="">添加订单</button>
...
export default {
    setup() {
        ...
        const pull_page = page => {
            ...
            api.getorderlist(params, store.state.user.token).then(res => {
                let resp = res.data;
                console.log(resp);
                if (resp.result === "success") {
                    order.value = resp.orders;
                    total = resp.count;
                    update_pages();
                }
            })
